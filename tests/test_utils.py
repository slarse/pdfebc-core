...
class UtilsTest(unittest.TestCase):
    ...
    def test_attach_valid_files(self):
        ...
        pdfebc_core.utils.attach_files(self.attachment_filenames, email_)
    ...
    def test_send_valid_email(self, mock_smtp):
        ...
        pdfebc_core.utils.send_email(email_, self.valid_config._sections)
    ...
    def test_send_valid_email_with_attachments(self, mock_smtp):
        ...
        pdfebc_core.utils.send_with_attachments(subject, message, self.attachment_filenames,
    ...
    def test_send_files_preconf_valid_files(self, mock_smtp):
        ...
        self.valid_config.write(self.temp_config_file)
        self.temp_config_file.close()
        pdfebc_core.utils.send_files_preconf(self.attachment_filenames, config_path=self.temp_config_file.name,
                                        status_callback=mock_status_callback)
        mock_smtp_instance.starttls.assert_called_once()
        mock_smtp_instance.login.assert_called_once_with(self.user, self.password)
        mock_smtp_instance.send_message.assert_called_once()
        mock_smtp_instance.quit.assert_called_once()
        expected_send_message = pdfebc_core.utils.SENDING_PRECONF.format(
            self.user, self.receiver,
            self.smtp_server, self.smtp_port, '\n'.join(self.attachment_filenames))
        expected_sent_message = pdfebc_core.utils.FILES_SENT
